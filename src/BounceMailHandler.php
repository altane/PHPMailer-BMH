...
class BounceMailHandler
{
	...
	const AUTH_CERT_VERIF = [0 => "/novalidate-cert", 1 => ""];
	...
	/**
	 * Imap connection verification certificat
	 *
	 * @var int
	 */
	public $authCertVerification = 1;
	...
	public function openMailbox(): bool
	{
		...
		if (!$this->testMode) {
			$this->mailboxLink = \imap_open('{' . $this->mailhost . ':' . $port . self::AUTH_CERT_VERIF[$this->authCertVerification] . '}' . $this->boxname, $this->mailboxUserName, $this->mailboxPassword, CL_EXPUNGE | ($this->testMode ? OP_READONLY : 0));
		} else {
			$this->mailboxLink = \imap_open('{' . $this->mailhost . ':' . $port . self::AUTH_CERT_VERIF[$this->authCertVerification] . '}' . $this->boxname, $this->mailboxUserName, $this->mailboxPassword, ($this->testMode ? OP_READONLY : 0));
		}
	...
	public function globalDelete(): bool
	{
		...
		$port = $this->port . '/' . $this->service . '/' . $this->serviceOption;
		$mboxt = \imap_open('{' . $this->mailhost . ':' . $port . self::AUTH_CERT_VERIF[$this->authCertVerification] . '}', $this->mailboxUserName, $this->mailboxPassword, OP_HALFOPEN);

		if ($mboxt === false) {
			return false;
		}

		$list = \imap_getmailboxes($mboxt, '{' . $this->mailhost . ':' . $port . self::AUTH_CERT_VERIF[$this->authCertVerification] . '}', '*');
		...
		if (\is_array($list)) {
			foreach ($list as $key => $val) {
				...
				if (false === \stripos($nameRaw, 'sent')) {
					$mboxd = \imap_open('{' . $this->mailhost . ':' . $port . self::AUTH_CERT_VERIF[$this->authCertVerification] . '}' . $nameRaw, $this->mailboxUserName, $this->mailboxPassword, CL_EXPUNGE);
	...
	public function mailboxExist(string $mailbox, bool $create = true): bool
	{
		...
		$port = $this->port . '/' . $this->service . '/' . $this->serviceOption;
		/** @noinspection PhpUsageOfSilenceOperatorInspection */
		$mbox = @\imap_open('{' . $this->mailhost . ':' . $port . self::AUTH_CERT_VERIF[$this->authCertVerification] . '}', $this->mailboxUserName, $this->mailboxPassword, OP_HALFOPEN);

		if ($mbox === false) {
			return false;
		}

		$list = \imap_getmailboxes($mbox, '{' . $this->mailhost . ':' . $port . self::AUTH_CERT_VERIF[$this->authCertVerification] . '}', '*');
		$mailboxFound = false;
		...
		if (\is_array($list)) {
			...
			if ($mailboxFound === false && $create) {
				/** @noinspection PhpUsageOfSilenceOperatorInspection */
				@\imap_createmailbox($mbox, \imap_utf7_encode('{' . $this->mailhost . ':' . $port . self::AUTH_CERT_VERIF[$this->authCertVerification] . '}' . $mailbox));
				\imap_close($mbox);
